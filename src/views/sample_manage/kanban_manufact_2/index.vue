      ...
      <a-spin
        ...
        <div class="" style="margin: 0 20px">
          <div class="left-side">
              ...
              <div style="display: flex">
                ...
                <div style="flex: 1.5;position: relative;">
                  <div style="right: 0;position: absolute;">
                    ...
                    <button id="stop" @click="stop" class="button button1"
                      >停止</button
                    >
                    &nbsp;&nbsp;
                    <button id="start" @click="start" class="button button2"
                      >播放</button
                    >&nbsp;&nbsp;
                    <button @click="next" class="button button3"
                      >下一界面</button
                    >
                  </div>
                </div>
            ...
            <div class="grid-demo-background" style="margin-top: 30px">
                          ...
                          <table style="width: 100%; height: 170px">
                            <tr style="width: 100%; height: 33%">
                              <td style="width: 35%" colspan="3"
                                ><div>当前样品计划完成双数</div> PLANNED QTY. OF
                                SAMPLE PROD.</td
                              >
                              <td style="width: 15%">{{ TOTALQTY1 }}</td>
                            </tr>
                            <tr style="width: 100%">
                              <td style="width: 35%"
                                ><div>实际完成数量</div>FINISHED QTY.</td
                              >
                              <td style="width: 15%">{{ TOTALFSQTY1 }}</td>
                              <td style="width: 35%"
                                ><div>完成比例(%)</div>PERCENTAGE OF
                                FINISH(%)</td
                              >
                              <td style="width: 15%">{{ FSP1 }}%</td>
                            </tr>
                            <tr style="width: 100%">
                              <td style="width: 35%"
                                ><div>RFT目标(%)</div>RFT TARGET(%)</td
                              >
                              <td style="width: 15%">99%</td>
                              <td style="width: 35%"
                                ><div>RFT实际达成(%)</div>RFT ACTUAL(%)</td
                              >
                              <td style="width: 15%">{{ PASSP1 }}%</td>
                            </tr>
                          </table>
                        </a-card>
                      </div>
                          ...
                          <table style="width: 100%; height: 170px">
                            <tr style="width: 100%; height: 33%">
                              <td style="width: 35%" colspan="3"
                                ...
                                SAMPLE PROD.</td
                              >
                              <td style="width: 15%">{{ TOTALQTY2 }}</td>
                            </tr>
                            <tr style="width: 100%">
                              <td style="width: 35%"
                                ><div>实际完成数量</div>FINISHED QTY.</td
                              >
                              <td style="width: 15%">{{ TOTALFSQTY2 }}</td>
                              <td style="width: 35%"
                                ><div>完成比例(%)</div>PERCENTAGE OF
                                FINISH(%)</td
                              >
                              <td style="width: 15%">{{ FSP2 }}%</td>
                            </tr>
                            <tr style="width: 100%">
                              <td style="width: 35%"
                                ><div>RFT目标(%)</div>RFT TARGET(%)</td
                          ...
                          <table style="width: 100%; height: 170px">
                            <tr style="width: 100%; height: 33%">
                              <td style="width: 35%" colspan="3"
                                ...
                                SAMPLE PROD.</td
                              >
                              <td style="width: 15%">{{ TOTALQTY3 }}</td>
                            </tr>
                            <tr style="width: 100%">
                              <td style="width: 35%"
                                ><div>实际完成数量</div>FINISHED QTY.</td
                              >
                              <td style="width: 15%">{{ TOTALFSQTY3 }}</td>
                              <td style="width: 35%"
                                ><div>完成比例(%)</div>PERCENTAGE OF
                                FINISH(%)</td
                              >
                              <td style="width: 15%">{{ FSP3 }}%</td>
                            </tr>
                            <tr style="width: 100%">
                              <td style="width: 35%"
                                ><div>RFT目标(%)</div>RFT TARGET(%)</td
...
<script lang="ts" setup>
  ...
  import useChartOption2 from '@/hooks/chart-option';
  import useLoading from '@/hooks/loading';
  import moment from 'moment';
  import { Getdatapie, Getdatatable } from '@/api/kanbansample';
  ...
  const TOTALQTY1 = ref<any>([]);
  const TOTALFSQTY1 = ref<any>([]);
  const FSP1 = ref<any>([]);
  const PASSP1 = ref<any>([]);
  const TOTALQTY2 = ref<any>([]);
  const TOTALFSQTY2 = ref<any>([]);
  const FSP2 = ref<any>([]);
  const PASSP2 = ref<any>([]);
  const TOTALQTY3 = ref<any>([]);
  const TOTALFSQTY3 = ref<any>([]);
  const FSP3 = ref<any>([]);
  const PASSP3 = ref<any>([]);
  ...
  const { chartOption3 } = useChartOption((isDark) => {
    return {
      label: {
        formatter: (params: any) => {
          if (params) {
  ...
  const search = async () => {
    ...
    try {
      ...
      const datalist1: any = await Getdatapie('B',startdate.value);
      data.value = datalist1.list;
      // console.log(datalist1.list);

      const datatable1: any = await Getdatatable('B',startdate.value);
      TOTALQTY1.value = datatable1.list[0].TOTALQTY;
      TOTALFSQTY1.value = datatable1.list[0].TOTALFSQTY;
      FSP1.value = datatable1.list[0].FSP;
      PASSP1.value = datatable1.list[0].PASSP;
      // console.log(TOTALQTY1.value);

      const datalist2: any = await Getdatapie('C',startdate.value);
      data1.value = datalist2.list;
      // console.log(datalist2.list);

      const datatable2: any = await Getdatatable('C',startdate.value);
      TOTALQTY2.value = datatable2.list[0].TOTALQTY;
      TOTALFSQTY2.value = datatable2.list[0].TOTALFSQTY;
      FSP2.value = datatable2.list[0].FSP;
      PASSP2.value = datatable2.list[0].PASSP;
      // console.log(TOTALQTY2.value);

      const datalist3: any = await Getdatapie('D',startdate.value);
      data2.value = datalist3.list;
      // console.log(datalist3.list);

      const datatable3: any = await Getdatatable('D',startdate.value);
      TOTALQTY3.value = datatable3.list[0].TOTALQTY;
      TOTALFSQTY3.value = datatable3.list[0].TOTALFSQTY;
      FSP3.value = datatable3.list[0].FSP;
      PASSP3.value = datatable3.list[0].PASSP;
      // console.log(TOTALQTY3.value);
    } catch (err) {
...
<style lang="less" scoped>
  ...
  ::v-deep .arco-card {
    ...
    box-shadow: rgba(20, 20, 20, 0.692) 0px 0px 10px inset;
    border-radius: 10px;
    border: none;
