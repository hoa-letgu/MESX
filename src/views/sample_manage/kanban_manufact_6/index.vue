          ...
          <div class="left-side">
            <div class="panel" style="height: 30px;margin: 0 22px;">
              <div style="display: flex;">
                ...
                <div style="flex: 1;text-align: center;margin-top: -20px;"> <h1 style="color:var(--color-text-1)">{{ $t('Kanban.list_manufact_6') }}</h1></div>
...
import { render } from 'nprogress';
  ...
  const columns = [
  {
    ...
    },
    {
      ...
      tooltip: true,
      width: 140,
      bodyCellStyle: (p:any)=>{ // console.log(p.ABN_CAUSE)
        if(p.ABN_CAUSE==="")
        {
          return {
  ...
  const search = async () => {
    ...
    try {
      setLocal();
      const data: any = await GetCutpagecount("E",startdate.value);
      // console.log(data.pagecount)
      if(pageindex>data.pagecount) pageindex = 1
      console.log("E")
      console.log(startdate.value)
      console.log(pageindex)
      const datalist1: any = await GetCutDetailtList(pageindex,"E",startdate.value);
