            ...
            <div class="container">
                <div class="container_outside">
                    <div class="feedbackContainer">
                        <!-- 头部 -->
                        <Vheader :title-msg="title" :pageIndex="pageIndex" @changepageIndex="changepageIndex"></Vheader>
                        <!-- 内容 -->
                        <div class="content" v-if="!pageIndex">
                            ...
                            <div class="searchContainer">
                                <div class="searchbox">
                                    <a-descriptions size="large" :column="4" border>
                                        <a-descriptions-item :label="'厂区'">
                                            <dia-table top="5vh" :head="headORG" ref="diaTable"
                                                :value="searchFrom[0].value" apiName="GeCQListDataAsc"
                                                @comfirm="GeCQListData"></dia-table>
                                        </a-descriptions-item>
                                        <a-descriptions-item :label="'验货员'">
                                            <dia-table top="5vh" :head="headWORK" ref="diaTable"
                                                :value="searchFrom[1].value" apiName="GetAQLInspector"
                                                @comfirm="GetAQLInspector"></dia-table>
                                        </a-descriptions-item>
                                        <a-descriptions-item :label="'出货国家'">
                                            <dia-table top="5vh" :head="CountryList" ref="diaTable"
                                                :value="searchFrom[2].value" apiName="GetGJistData"
                                                @comfirm="GetCountryListData"></dia-table>
                                        </a-descriptions-item>
                                        <a-descriptions-item :label="'系列'">
                                            <dia-table top="5vh" :head="DevSeriesList" ref="diaTable"
                                                :value="searchFrom[3].value" apiName="GetDevelopmentSeries"
                                                @comfirm="GetDevSeriesListData"></dia-table>
                                        </a-descriptions-item>
                                        <a-descriptions-item :label="'问题级别'">
                                            <a-select v-model="searchFrom[4].value" placeholder="请选择" size="large"
                                                style="width: 260px">
                                                <a-option v-for="item in optionsSelect" :key="item.value"
                                        ...
                                        <a-descriptions-item :label="'验货日期'">
                                            ...
                                            <span class="inspectionPicker">
                                                <a-date-picker v-model="searchFrom[5].value" show-time
                                                    :timepicker-props="{ defaultValue: '09:09:06' }"
                                                    :placeholder="'选择时间'" format="YYYY-MM-DD HH:mm:ss" size="large"
                                                    required></a-date-picker>
                                            </span>
                                            <span>&nbsp;&nbsp;~&nbsp;&nbsp; </span>
                                            <span class="inspectionPicker">
                                                <a-date-picker v-model="searchFrom[6].value" :placeholder="'选择时间'"
                                                    show-time :timepicker-props="{ defaultValue: '09:09:06' }"
                                                    format="YYYY-MM-DD HH:mm:ss" size="large" required></a-date-picker>
                                            </span>
                                        </a-descriptions-item>
                                    </a-descriptions>
                                </div>
                                <div class="search" style="text-align: right">
                                    <a-button type="primary" @click="search" size="large" style="margin-right: 10px">
                                        <template #icon>
                                            <icon-search />
                                        </template>

                                        <template #default>开始搜索</template>
                                    </a-button>
                                    <a-button @click="reload" size="large">
                                        <template #icon>
                                            <icon-refresh />
                                        </template>

                                        <template #default> 重置搜索</template>
                                    </a-button>
                                </div>
                            </div>
                            <div class="contentBox">
                                ...
                                <div class="contentLeft common common1">
                                    <div class="leftTop commontitle" @click="GotoMainDetail('1', 'PODD验货明细')"
                                        style="height: 320px; border: 1px">
                                        <div class="headtitle commonflex">今日PODD验货分析</div>
                                        <div class="common">
                                            <dv-border-box-13 class="dv-border-box-12" :color="['#0bc0c4', '#0bc0c4']">
                                                <dv-scroll-board :config="POddconfig" class="board-info"
                                                    style="width: 436px;height: 270px;" />
                                            </dv-border-box-13>
                                        </div>
                                    </div>
                                    <div class="leftMid common commontitle">
                                        <div class="headtitle commonflex">前十问题点</div>
                                        <div class="common">
                                            <dv-border-box-13 class="dv-border-box-12" :color="['#0bc0c4', '#0bc0c4']">
                                                <a-table :columns="columns" :pagination="false" :hoverable="false"
                                                    :bordered="false" :row-class="tableRowClassName"
                                                    :data="proDatabledata" size="large">
                                ...
                                <div class="contentMid common1">
                                    <div class="midT">
                                        <div class="chartTitle">
                                            <div :style="[MonthYear ? commoncheck : '']" @click="changeMonthYear(1)">
                                                月度验货分析</div>
                                            <div class="midgap"></div>
                                            <div :style="[!MonthYear ? commoncheck : '']" @click="changeMonthYear(2)">
                                                年度验货分析</div>
                                        </div>
                                        <!-- 年月度分析表 -->
                                        <div style="width: 900px;" class="midchart">
                                            <div @click="GotoMainDetail('2', '月度验货分析')"
                                                style="width: 900px;height: 100%;" v-if="MonthYear">

                                                <VCharts :option="optionMonth" style="width: 100%;height: 530px"
                                                    ref="monthChart">
                                                </VCharts>
                                            </div>
                                            <div style="width: 900px;height: 100%;" v-if="!MonthYear"
                                                @click="GotoMainDetail('3', '年度验货分析')">

                                                <VCharts :option="yearOption" style="width: 100%;height: 530px"
                                                    ref="yearChart">
                                                </VCharts>
                                            </div>
                                    ...
                                    <div class="midB">
                                        <div class="midL common  commontitle" @click="GotoMainDetail('4', '验货员分析')">
                                            <div class="headtitle commonflex">验货员分析</div>
                                            <div class="common">
                                                <dv-border-box-13 class="dv-border-box-12 checkpeople "
                                                    :color="['#0bc0c4', '#0bc0c4']">

                                                    <div>
                                                        <div style="width: 420px;height: 20px;" class="tablerow">
                                                            <!-- <p>验货员</p> -->
                                                            <div
                                                                style="display: flex;flex-direction: row;justify-content: center;align-items: center;margin-right: 10px">
                                                                <div
                                                                    style="width: 10px;height: 10px;background-color: #46df41;">
                                                                </div>
                                                                <p>
                                                                    合格数
                                                                </p>
                                                            ...
                                                            <div
                                                                ...
                                                                <div
                                                                    style="width: 10px;height: 10px;background-color: #f65e52;">
                                                                </div>
                                                                <p>
                                                                    不合格数
                                                                </p>
                                                        ...
                                                        <dv-scroll-board :config="checkPeople" class="board-info"
                                        ...
                                        <div class="midR common  commontitle" @click="GotoMainDetail('5', '出货国家分析')">
                                            <div class="headtitle commonflex">出货国家分析</div>
                                            ...
                                            <div class="common">
                                                <dv-border-box-13 class="dv-border-box-12 checkpeople "
                                                    ...
                                                    <div>
                                                        ...
                                                        <dv-scroll-board :config="checkcountry" class="board-info "
                                                            style="width:432px;height: 210px" />
                                                    </div>
                                ...
                                <div class="contentRight common common1">
                                    <div class="rightTop common commontitle" @click="GotoMainDetail('6', '本月订单明细')">
                                        <div class="headtitle commonflex">本月订单分析</div>
                                        <div class="common">
                                            <dv-border-box-13 class="dv-border-box-12 cmmon1"
                                                ...
                                                <div class="oder-all">
                                                    <div class=" oderanay">
                                                        <div class=" order-top">
                                                            <P>{{
                                                                monthOrder.monthOrderCount?monthOrder.monthOrderCount.toLocaleString('zh-CN'):0
                                                                }}
                                                                个</P>
                                                            <P>本月订单数</P>
                                                        </div>
                                                        <div class=" order-top">
                                                            <P>{{
                                                                monthOrder.monthOrderSeQty?monthOrder.monthOrderSeQty.toLocaleString('zh-CN'):0
                                                                }}
                                                                个</P>
                                                            <P>本月订单已验货</P>
                                                        </div>
                                                    ...
                                                    <div class=" oderanay">
                                                        <div class="order-mid">
                                                            <P>{{
                                                                monthOrder.monthTotalSeQty?monthOrder.monthTotalSeQty.toLocaleString('zh-CN'):0
                                                                }}
                                                                双</P>
                                                            <P>本月总订单</P>
                                                        </div>
                                                        <div class="order-mid">
                                                            <P>{{
                                                                monthOrder.monthTotalSeQtyInspected ? monthOrder.monthTotalSeQtyInspected.toLocaleString('zh-CN') : 0
                                                            }} 双</P>
                                                            <P>本月订单已验货</P>
                                                        </div>
                                                    </div>
                                                    <div class=" oderanay">
                                                        <div class="order-bottom">
                                                            <P>{{
                                                                monthOrder.monthCNOrderSeQty ? monthOrder.monthCNOrderSeQty.toLocaleString('zh-CN') : 0
                                                                }} 双</P>
                                                            <P>本月中国订单</P>
                                                        </div>
                                                        <div class="order-bottom">
                                                            <P>{{
                                                                monthOrder.monthCNOrderSeQtyInspected ?
                                                                    monthOrder.monthCNOrderSeQtyInspected.toLocaleString('zh-CN') : 0
                                                                }} 双</P>
                                                            <P>本月中国订单已验货</P>
                                                        </div>
                                                    </div>
                                    ...
                                    <div class="rightTopMid common commontitle">
                                        <div class="headtitle commonflex">验货总览</div>
                                        <div class="common">
                                            <dv-border-box-13 class="dv-border-box-12" :color="['#0bc0c4', '#0bc0c4']">
                                                <dv-scroll-ranking-board :config="checkGoodsconfig"
                                                    class="ranking-info rmidrank" style="width:350px;height: 220px;" />
                                            </dv-border-box-13>
                                        </div>

                                    </div>

                                    <div class="rightTopBottom common commontitle"
                                        @click="GotoMainDetail('7', '厂区验货分析')">
                                        <div class="headtitle commonflex">厂区验货分析</div>
                                        <div class="common">
                                            <dv-border-box-13 class="dv-border-box-12 checkpeople"
                                                :color="['#0bc0c4', '#0bc0c4']">
                                                <div>
                                                    ...
                                                    <dv-scroll-board :config="checkfactory" class="board-info"
                        ...
                        <div class="returnDetail" v-if="pageIndex">
                            <div class="content">
                                <div class="buttonrow" v-if="ispodd">
                                    <a-button :style="[istoday == 1 ? poddButton : '']" size="large"
                                        @click="getPoddDteail(1)">今日</a-button>
                                    <a-button :style="[istoday == 2 ? poddButton : '']" size="large"
                                        @click="getPoddDteail(2)">本周</a-button>
                                    <a-button :style="[istoday == 3 ? poddButton : '']" size="large"
                                        @click="getPoddDteail(3)">本月</a-button>
                                    <a-button type="primary" status="success"
                                        @click="exportInventoryDetail">导出</a-button>

                                </div>
                                <a-table :columns="headlist" :data="tableData" :pagination="false"
                                    :default-page-size='17' :hoverable="false" :bordered="false" size="large"
                                    :row-class="tableRowClassName" />

                            </div>
                            <a-pagination :total="dataTotal" :page-size-options="[5, 10, 17]" show-total show-jumper
                                show-page-size :default-page-size='17' class="pagechange" @change="handleCurrentChange"
                                @page-size-change="handleSizeChange" :current="currentPage" :page-size="pageSize" />
                        </div>
                    </div>
                </div>
            </div>
...
import { GetPODDInfo, GetTop10Issues, GetMonthInspectionAnalysis, GetYearInspectionAnalysis, GetInspectorAnalysis, GetDescountryAnalysis, GetCrrentMonthOrderInfo, GetAQLInspectionOverview, GetPlantAnalysis, GetCrrentMonthOrderDetail } from '@/api/aeqs';
import { formatDateTime, formatNowDate } from '@/utils/date';
import Vheader from './compents/compoents/Vheader.vue';


const title = ref<string>('AQL分析');
...
columns.value = [
    ...
    {
        title: '问题级别', dataIndex: 'PROBLEM_LEVEL', align: 'center', with: 110

    },
    {
        title: '问题点', dataIndex: 'BAD_ITEM_NAME', align: 'center', ellipsis: true,
        tooltip: true,
    },
    {
        title: '数量', dataIndex: 'PROBLEM_COUNT',
        align: 'center',
    ...
    {
        title: '百分比', dataIndex: 'RATE', align: 'center'
    },
]
...
const getPODDInfo = async (dataMain: any) => {
    ...
    res.data.RetData1.dataTable.forEach((item: any) => {
        ...
        detailitem["序号"] = item.RN
        detailitem.SO = item.SE_ID
        detailitem["PO分组"] = item.MER_PO
        detailitem.PO = item.CUSTOMER_PO
        detailitem.PODD = item.PODD
        detailitem["状态"] = item.STATUS
        detailitem["已验货/未验货数量"] = item.INSPECTION_RATE
        detailitem["验货员"] = item.INSPECTOR
...
const getInspectorAnalysis = async (dataMain: any) => {
    ...
    res.data.RetData1.HisData.x.map((item: any, index: any) => {
        const checkitem = [item, res.data.RetData1.HisData.bar[0].data[index], res.data.RetData1.HisData.bar[1].data[index]]
    ...
    checkdt.map((item: any, index: any) => {
        item[0] = `
          <div style="height:20px;line-height:20px;overflow:hidden;">
      ...
      display: flex;
        justify-content: flex-end;">
            <div style="width:${item[1] / 180 * 100 + 10}%;background-color:#46df41;height:20px;line-height:20px;overflow:hidden;">
      ...
      display: flex;
        justify-content: flex-start;margin-left:10px">
          <div style="width:${item[2] / 180 * 100 + 10}%;background-color:#f65e52;height:20px;line-height:20px;overflow:hidden">
    ...
    res.data.RetData1.dataTable.forEach((item: any, index: any) => {
        ...
        detailitem["序号"] = index + 1
        detailitem["验货员"] = item.STAFF_NAME;
        detailitem["合格"] = item.ACCEPT_QTY;
        detailitem["不合格"] = item.REJECT_QTY;
        detailitem["任务总数"] = item.SUM_QTY;
        detailitem["合格率"] = `${item.ACCEPT_RATE_PCT} % `
        tabledt.push(detailitem)
    })
...
const getMonthInspectionAnalysis = async (dataMain: any) => {
    ...
    const alldata = [...res.data.RetData1.HisData.bar[0].data, ...res.data.RetData1.HisData.bar[1].data]
    optionMonth.value.yAxis[0].max = Math.max(...alldata)
    optionMonth.value.yAxis[0].interval = Math.ceil((Math.max(...alldata) + 10) / 10)
    ...
    res.data.RetData1.dataTable.forEach((item: any, index: any) => {
        ...
        detailitem["序号"] = index + 1
        detailitem["月份"] = item.COMMIT_DATE;
        detailitem["合格"] = item.ACCEPT_QTY;
        detailitem["不合格"] = item.REJECT_QTY;
        detailitem["任务总数"] = item.SUM_QTY;
        detailitem["合格率"] = `${item.ACCEPT_RATE_PCT}%`
...
const getYearInspectionAnalysis = async (dataMain: any) => {
    ...
    yearOption.value.yAxis[0].interval = Math.ceil((Math.max(...alldata) + 10) / 10)
    if (interval > 0) {
        yearOption.value.xAxis[0].max = interval - 1;
        yearOption.value.xAxis[0].interval = Math.ceil(interval - 1) / (interval - 1)
        yearOption.value.xAxis[1].max = interval - 1;
        yearOption.value.xAxis[1].interval = Math.ceil(interval - 1) / (interval - 1)
        ...
        yearOption.value.xAxis[5].data = res.data.RetData1.HisData.line[0].data.map((item: any) => {
            return `${item} %`
        })
...
const getDescountryAnalysis = async (dataMain: any) => {
    ...
    const faildata = res.data.RetData1.HisData.bar[1].data
    res.data.RetData1.HisData.x.map((item: any, index: any) => {
        const countryitem = [item, res.data.RetData1.HisData.bar[0].data[index], res.data.RetData1.HisData.bar[1].data[index]]
        const dataitem = { 'name': item, 'value': sucessdata[index] }
        const dataitem1 = { 'name': item, 'value': faildata[index] }
        sucessdt.push(dataitem)
        faildat.push(dataitem1)
    ...
    res.data.RetData1.dataTable.forEach((item: any, index: any) => {
        ...
        detailitem["序号"] = index + 1
        detailitem["国家"] = item.DESCOUNTRY_NAME;
        detailitem["合格"] = item.ACCEPT_QTY;
        detailitem["不合格"] = item.REJECT_QTY;
        detailitem["任务总数"] = item.SUM_QTY;
        detailitem["合格率"] = `${item.ACCEPT_RATE_PCT}%`
...
const getAQLInspectionOverview = async (dataMain: any) => {
    ...
    checkGoodsconfig.value.data = [
        { name: '任务总数', value: overviewData.taskNum },
        { name: '合格任务数', value: overviewData.acceptNum },
        { name: '不合格任务数', value: overviewData.rejectNum },
        { name: '合格率', value: overviewData.acceptanceRate },
        { name: '不合格率', value: overviewData.rejectionRate },
    ]
}
/*
*厂区验货
...
const getPlantAnalysis = async (dataMain: any) => {
    ...
    const faildata = res.data.RetData1.HisData.bar[1].data
    res.data.RetData1.HisData.x.map((item: any, index: any) => {
        const factoryitem = [item, res.data.RetData1.HisData.bar[0].data[index], res.data.RetData1.HisData.bar[1].data[index]]
        const dataitem = { 'name': item, 'value': sucessdata[index] }
        const dataitem1 = { 'name': item, 'value': faildata[index] }
        sucessdt.push(dataitem)
        faildat.push(dataitem1)
    ...
    factorydt.map((item, index) => {
      ...
      display: flex;
        justify-content: flex-end;">
            <div style="width:${item[1] / 180 * 100 + 10}%;background-color:#46df41;height:20px;line-height:20px;overflow:hidden;display: flex;justify-content: center">
              <span style="color:#ffffff">${item[1]}</span></div>  
            </div>
      ...
      display: flex;
        justify-content: flex-start;margin-left:10px">
          <div style="width:${item[2] / 180 * 100 + 10}%;background-color:#f65e52;height:20px;line-height:20px;overflow:hidden;display: flex;justify-content: center">
              <span style="color:#ffffff">${item[2]}</span></div>     
              </div>  
          `
        return item
...
const getCrrentMonthOrderDetail = async (dataMain: any) => {
    ...
    res.data.RetData1.dataTable.forEach((item: any, index: any) => {
        ...
        detailItem["序号"] = item.RN
        detailItem["目的地"] = item.DESCOUNTRY_NAME
        detailItem["本月订单数"] = item.TOTAL_QTY
        detailItem["已验货订单数"] = item.INSPECTED_QTY
        detailItem["未验货订单数"] = item.UNINSPECTED_QTY
        tabledt.push(detailItem)
    })
    if (tabledt.length > 0) {
        dataTotal.value = res.data.RetData1.total
...
async function GotoMainDetail(type: any, Itemtitle: any) {
    ...
    if (type === '1') {
        ...
        if (detailDatas.value[1].length > 0) {
            dataTotal.value = res.data.RetData1.total
            const headt = Object.keys(detailDatas.value[1][0]).filter(item => item);
            headt.forEach((item, index) => {
                const headtItem = {
                    title: item,
                    dataIndex: item,
...
function getPageInfo() {
    ...
    for (let i = (currentPage.value - 1) * pageSize.value; i < dataTotal.value; i += 1) {
...
function exportInventoryDetail() {
    ...
    const Poddcolumns = [
        ...
        { header: 'PO分组', key: 'PO分组', width: 15 },
        { header: 'PO', key: 'PO', width: 15 },
        { header: 'PODD', key: 'PODD', width: 50 },
        { header: '状态', key: '状态', width: 15 },
        { header: '已验货/未验货数量', key: '已验货/未验货数量', width: 15 },
        { header: '验货员', key: '验货员', width: 15 },

    ];
    // 将时间字符串拼接到文件名中
    const fileName = `PODD验货明细${formatNowDate('yyyyMMddHHmmss')}.xlsx`;
...
function InsertReportLog(param: any) {
    ...
    const base = defaultSettings.api_url.substring(0, defaultSettings.api_url.indexOf('CommonCall'));
